...
fun buildGrades(grades: Map<String, Int>): Map<Int, List<String>> {
    val memory = mutableMapOf<Int, List<String>>()
    grades.values.toSet().forEach { memory[it] = grades.filter { (_, v) -> v == it }.keys.toList() }
    return memory
}
...
fun mergePhoneBooks(mapA: Map<String, String>, mapB: Map<String, String>): Map<String, String> {
    ...
    (mapA.keys + mapB.keys).forEach {
        when {
            it in mapA && it in mapB && mapA[it] != mapB[it] -> mapC[it] = "${mapA[it]}, ${mapB[it]}"
            it in mapA && it in mapB && mapA[it] == mapB[it] -> mapC[it] = mapA[it]!!
            it in mapA && it !in mapB -> mapC[it] = mapA[it]!!
            it !in mapA && it in mapB -> mapC[it] = mapB[it]!!
        }
    }
    return mapC
}
...
fun averageStockPrice(stockPrices: List<Pair<String, Double>>): Map<String, Double> {
    val price = mutableMapOf<String, Pair<Double, Int>>()
    for ((k, v) in stockPrices) {
        val pair = price.getOrDefault(k, 0.0 to 0)
        price[k] = pair.first + v to pair.second + 1
    }
    val finish = mutableMapOf<String, Double>()
    price.forEach { (k, v) -> finish[k] = v.first / v.second }
    return finish
}
...
fun extractRepeats(list: List<String>): Map<String, Int> {
    ...
    list.forEach { repeats[it] = repeats.getOrDefault(it, 0) + 1 }
...
fun hasAnagrams(words: List<String>): Boolean {
    ...
    words.forEach {
        val word = it.toList().sorted()
        if (word in sortedWords) return true else sortedWords.add(word)
    }
    return false
}
...
fun findSumOfTwo(list: List<Int>, number: Int): Pair<Int, Int> {
    ...
    list.forEachIndexed { index, i ->
        if (number - i in memory) return Pair(memory[number - i]!!, index) else memory[i] = index
    }
    return Pair(-1, -1)
}
